    ...
    return (
        <Card className="h-[380px] flex flex-col hover:shadow-lg transition-all duration-300 border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 group">
            <CardHeader className="pb-2 shrink-0">
                {/* Title & Link */}
                <div className="flex justify-between items-start gap-2 h-[56px]">
                    <CardTitle className="text-lg font-bold leading-tight text-neutral-900 dark:text-neutral-100 line-clamp-2">
            ...
            <div className="w-full h-1 flex my-1 px-6 opacity-80 shrink-0">
            ...
            <CardContent className="pt-3 pb-2 flex-1 overflow-hidden flex flex-col">
                <Tabs defaultValue="summary" className="w-full h-full flex flex-col">
                    <TabsList className="grid w-full grid-cols-3 mb-2 bg-slate-100 dark:bg-slate-800 h-8 p-1 shrink-0">
                        <TabsTrigger value="summary" className="text-xs">{t('Summary') || 'Summary'}</TabsTrigger>
                        <TabsTrigger value="interpretation" className="text-xs">{t('Interpretation') || 'Interpretation'}</TabsTrigger>
                        <TabsTrigger value="comments" className="text-xs">{t('Comments') || 'Comments'}</TabsTrigger>
                    </TabsList>
                    ...
                    <div className="flex-1 overflow-y-auto pr-1">
                            ...
                            {story.summary ? (
                                <ul className="space-y-2">
                                    {story.summary.map((item, i) => (
                                        <li key={i} className="text-sm text-slate-600 dark:text-slate-400 flex items-start gap-2 leading-relaxed">
                                ...
                                <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground gap-2">
                                    <p className="text-xs italic">{t('AI Summary not available yet.')}</p>
                                </div>
                            )}
                        </TabsContent>
                        ...
                        <TabsContent value="interpretation" className="mt-0 h-full">
                            {story.interpretation ? (
                                ...
                                <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground gap-2">
                                    <p className="text-xs italic">{t('Interpretation pending.')}</p>
                                </div>
                            )}
                        </TabsContent>
                        ...
                        <TabsContent value="comments" className="mt-0 h-full">
                            {story.aiComments ? (
                                ...
                                <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground gap-2">
                                    <p className="text-xs italic">{t('AI Comment analysis pending.')}</p>
                                </div>
                            )}
                        </TabsContent>
                    </div>
            ...
            <CardFooter className="pt-0 pb-3 h-[40px] shrink-0 flex items-center justify-between border-t border-dashed border-slate-100 dark:border-slate-800 mt-auto">
                {/* Category Badge - Show 'General' if missing */}
                <Badge variant="secondary" className="bg-slate-50 text-slate-500 border border-slate-100 hover:bg-slate-100 dark:bg-slate-800/50 dark:text-slate-400 dark:border-slate-800 px-2 py-0 rounded-full font-normal text-[10px]">
                ...
                {story.keywords && story.keywords.length > 0 && (
                    <div className="flex gap-1 overflow-hidden justify-end">
                        {story.keywords.slice(0, 2).map((kw, i) => (
                            <Badge key={i} variant="outline" className="text-[10px] text-slate-400 dark:text-slate-500 font-normal border-slate-100 dark:border-slate-800 px-1.5 py-0 whitespace-nowrap">
                                {kw}
                            </Badge>
                        ))}
                    </div>
